...
public class GraphOverviewController implements Initializable {
    ...
    public void getMas()
    {
        System.out.println(mChannelsArrayList.size());
        for (int i = 0; i < mChannelsArrayList.size(); i++) {
            ...
            textField.setId("txt" + channelNumber);
            textField.setOnKeyPressed(e->actionChangeTextField(e));
    ...
    private void actionChangeTextField(KeyEvent  e) {
        TextField textField = (TextField) e.getSource();
        String textFieldIdx = textField.getId().replace("txt", "");
        if(e.getCode().equals(KeyCode.ENTER)) {
            //System.out.println(textField.getId());
            //System.out.println(textField.getText());
            
            int intId = Integer.parseInt(textFieldIdx) - 1;
            int value = Integer.parseInt(textField.getText());
            Channel channel = mChannelsArrayList.get(intId);
            
            Series currentSeries = mSeriesList.get(intId);
            
            ObservableList<XYChart.Data> channelObservableList =
                    FXCollections.observableArrayList();
            for(int i_channelDataItem = 0; i_channelDataItem < channel.size() - 1; i_channelDataItem++) {
                Double v = channel.getDataItem(i_channelDataItem) * value;
                channelObservableList.add(new XYChart.Data(i_channelDataItem, v));
            }
            currentSeries.setData(channelObservableList);
        }
    }
